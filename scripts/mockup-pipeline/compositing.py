...
- compose_strict_bg_only(): like compose_light but with a much stricter
  white threshold (R>252 & G>252 & B>252) and a narrow 2-px feather. Designed
  for shorts/maillots/claquettes whose Printify mockups now ship with real
  colors — the subject must stay 100% intact and only the pure-white studio
  backdrop gets replaced.
"""
import io
...
def compose_strict_bg_only(
    ...
    """Replace ONLY the pure-white studio backdrop with the collection bg.

    Use for products that already carry their real colors in the Printify
    mockup (shorts, maillots, claquettes after user-side color fix). The
    threshold is intentionally strict (>=253) so any printed color, fabric
    fold, shadow, or strap stays untouched. Only background-connected pixels
    are swapped — enclosed white inside the product (e.g. drawstring eyelets)
    is preserved.
    """
